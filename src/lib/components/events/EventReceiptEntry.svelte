<script>
	...
	const eventIntrinsics = Object.keys(event ?? {}).reduce(
		(intrinsics, key) => {
			if (eventHiddenMembers.includes(key)) delete intrinsics[key];
			return intrinsics;
		},
		{ ...event }
	);
...
<Card size="xl">
    ...
    -->
		...
		<div>
			<Hr />
			<Toggle bind:checked={revealRawEventIntrinsics} class="mt-4 italic dark:text-gray-500"
				>Raw Event Intrinsics (who/what/when)</Toggle
			>
			<Toggle bind:checked={revealRawEventAttributes} class="mt-4 italic dark:text-gray-500"
				>Raw Event Attributes</Toggle
			>
			<Toggle bind:checked={revealRawAssetAttributes} class="mt-4 italic dark:text-gray-500"
				>Raw Asset Attributes</Toggle
			>
