...
export async function getReceipt(eventIdentity, blockNumber=undefined, publicProof=undefined) {
  ...
  */
  ...
  let url = (
    '/api/' + env['PUBLIC_RKVST_IO_SEGMENT'] + '/' + notaryEventClaimsPath
  );
  ...
  url = (
    '/api/' + env['PUBLIC_RKVST_IO_SEGMENT'] + '/' + notaryEventReceiptsPath
  );
  ...
  try {
    ...
    console.log(`ERROR: fetching receipt for ${eventIdentity} with claim "${claim}" ${resp.statusText} ${err}`);
    throw(err);
  }
  if (!receipt)
    throw new Error(`ERROR: empty receipt from ${notaryEventReceiptsPath} for ${eventIdentity} claim "${claim}"`);
  return receipt;
}